...
function broadcastRoomState(roomId){
  ...
  const safePlayers = r.players.map(p => ({ id: p.id, name: p.name, count: p.hand.length, isHost: p.isHost || false, finished: p.finished || false }));
...
io.on('connection', socket => {
  ...
  socket.on('join_room', ({ roomId, name }, cb) => {
    ...
    if (r.players.find(p => p.socketId === socket.id)) return cb && cb({ ok: false, error: 'Already joined' });
  ...
  socket.on('start_game', ({ roomId }, cb) => {
    ...
    r.started = true; r.turnIndex = 0; r.pile = []; r.lastClaim = null; 
  ...
  socket.on('play_cards', ({ roomId, playerId, cardIds, claim }) => {
    ...
    // Prevent single-player repeated moves
    const activePlayers = r.players.filter(p => !p.finished);
    if(activePlayers.length === 1 && r.players[idx].hand.length > 0) {
      socket.emit("error", "Wait — you can't play again until others join or game ends!");
      return;
    }
    ...
    if(player.hand.length === 0) {
      ...
      socket.emit("finished_hand", { message: "You finished your hand! Wait for the rest." });
  ...
  socket.on('call_bluff', ({ roomId, callerId, claimedPlayerId }, cb) => {
    ...
    // Mark finished status correctly
    if(r.players[claimedIdx].hand.length === 0) r.players[claimedIdx].finished = true;
    if(r.players[callerIdx].hand.length === 0) r.players[callerIdx].finished = true;
  ...
  socket.on('leave_room', ({ roomId }) => { 
    ...
    socket.leave(roomId); 
    if(rooms[roomId]) broadcastRoomState(roomId); 
  });
